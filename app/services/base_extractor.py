...
class BaseExtractor:
    ...
    def find_value_near_label(
        ...
        *,
        ...
        Args:
            texts:             Sorted OCR text list.
            label_keywords:    List of strings to search as labels (case-insensitive).
            direction:         Where to look for the value relative to the label.
                               "auto" = right first, then below.
            max_distance:      Maximum pixel distance to consider.
            same_row_tolerance: Vertical tolerance for "same row" detection.
            min_confidence:    Minimum OCR confidence for value candidates.
            skip_label_chars:  Strip leading ":/-" from found value.
    ...
    def find_by_regex(
        self, texts: List[OCRText], pattern: str, flags: int = re.IGNORECASE
    ) -> Optional[str]:
    ...
    def _find_label_box(
        self, texts: List[OCRText], keywords: List[str]
    ) -> Optional[OCRText]:
        """
        ...
        for item in texts:
            ...
            for kw in kw_upper:
                ...
                if t == kw_clean or kw_clean in t or t in kw_clean:
                    return item
